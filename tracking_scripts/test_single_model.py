...
def test_pipeline(net, test_array, algo):
    results = {}
    results['gcn'] = {
        't1': [],
        't2': [],
    ...
    for file in tqdm.tqdm(test_array, desc='testing', position=0, leave=True):
        ...
        # try:
        yhat = net.predict_assignment(graph, assignment_method=algo).flatten()
        y = graph.y.squeeze().cpu().numpy()
        conf_matrix = confusion_matrix(y, yhat)
        if conf_matrix.shape == (1,1):
            conf_matrix = np.array([[conf_matrix[0,0], 0], [0,0]]) # if there is only one class in the data
        results['gcn']['confusion'].append(conf_matrix)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--assgraphs', dest='assgraphs', required=True, type=str, help='assgraphs to test on')
    ...
    dataset_regex={
        ...
        'fission_5': "^240102_30C_fig_(SW182|SX387)_(01|02|03|05)_segmentation__assgraph__dt_00(1)__.*pt$",
        '05': "^240102_30C_fig_(SW182|SX387)_(05)_segmentation__assgraph__dt_00(1)__.*pt$",
        ...
        'fission_wt_dt_1': "^wt_pom1D_01_(07|15|20|30)_R3D_REF_dv_trk_segmentation__assgraph__dt_00(1)__.*pt$", # all fission files excepr for sw182_02
        'debug': '^wt_pom1D_01_20_R3D_REF_dv_trk_segmentation__assgraph__dt_00(1)__.*pt',
        'fission_all_dt_1':  "^(wt_pom1D_01_(15|30)_R3D_REF_dv_trk_segmentation|240102_30C_fig_(SW182|SX387)_(01|02|03|05)_segmentation)__assgraph__dt_00(1)__.*pt$", # all fission files (02 and 30 is new)
        'fission_wt_30_dt_1': "^wt_pom1D_01_(30)_R3D_REF_dv_trk_segmentation__assgraph__dt_00(1)__.*pt$", # all fission files excepr for sw182_02
        'fission_wt_07_dt_1': "^wt_pom1D_01_(07)_R3D_REF_dv_trk_segmentation__assgraph__dt_00(1)__.*pt$", # all fission files excepr for sw182_02
        'fission_wt_15_dt_1': "^wt_pom1D_01_(15)_R3D_REF_dv_trk_segmentation__assgraph__dt_00(1)__.*pt$", # all fission files excepr for sw182_02
    }
    ...
    for model, assgraphs in model_dirs.items():
        file_array = []
        ...
        res_details = test_pipeline(new_net, file_array, args.algo)
